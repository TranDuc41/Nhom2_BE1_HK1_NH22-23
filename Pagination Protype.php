...
class Pagination
{
    ...
    private function getPrePage()
    {
        ...
        if ($this->getCurrentPage() === 1) {
            ...
            return '<li><a href="' . $_SERVER['PHP_SELF'] . '?' . $this->config['querystring'] . '=' . ($this->getCurrentPage() - 1) . '&type_id'.'='.$_GET['type_id']. '" >Pre</a></li>';
        }
    }
    ...
    private function getNextPage()
    {
        ...
        if ($this->getCurrentPage() >= $this->gettotalPage()) {
            ...
            // còn không thì trả về HTML code
            return '<li><a href="' . $_SERVER['PHP_SELF'] . '?' . $this->config['querystring'] . '=' . ($this->getCurrentPage() + 1) .'&type_id'.'='.$_GET['type_id']. '" >Next</a></li>';
        }
    }
        ...
        if (isset($this->config['full']) && $this->config['full'] === false) {
            ...
            for ($i = ($this->getCurrentPage() - 3) > 0 ? ($this->getCurrentPage() - 3) : 1; $i <= (($this->getCurrentPage() + 3) > $this->gettotalPage() ? $this->gettotalPage() : ($this->getCurrentPage() + 3)); $i++) {
                if ($i === $this->getCurrentPage()) {
                    ...
                    $data .= '<li><a href="' . $_SERVER['PHP_SELF'] . '?' . $this->config['querystring'] . '=' . $i .'&type_id'.'='.$_GET['type_id']. '" >' . $i . '</a></li>';
                }
            }
